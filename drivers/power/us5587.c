...
u8 us5587_dump_value(void)
{
	...
	printk("[BAT][CHG] adapter_AIN2 voltage value = 0x%xh\n", my_read_value);

	return my_read_value;
}
EXPORT_SYMBOL(us5587_dump_value);

u8 thermal_AIN1_dump_value(void)
{
	u8 my_read_value = 0;

	us5587_read_reg(us5587_raddr, 0x03, &my_read_value);
	//printk("[BAT][CHG] thermal_AIN1 voltage value = 0x%xh\n", my_read_value);

	return my_read_value;
}
EXPORT_SYMBOL(thermal_AIN1_dump_value);

static ssize_t thermal_AIN1_value_show(struct device *dev, struct device_attribute *da,
	char *buf)
{
	u16 ret;

	ret = thermal_AIN1_dump_value();

	return sprintf(buf, "0x%xh\n", ret);
}

static DEVICE_ATTR(thermal_AIN1_value, 0664, thermal_AIN1_value_show, NULL);

static struct attribute *dump_reg_attrs[] = {
	&dev_attr_thermal_AIN1_value.attr,
	NULL
};
