...
//ST proc for CE catch part of K data (10 procs)+++

static ssize_t dump_ST_debug_value1_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	int val;
	char messages[8]="";
	if (len > 8)	len = 8;
	...
	if(val > 0)
		g_time_budget_ms = val;

	printk("STlog time budget(%d)\n",g_time_budget_ms);
	
	return len;
}


static int dump_ST_debug_value1_read(struct seq_file *buf, void *v)
{
	int8_t status = 0;
	...
	if(status){
		printk("%s: fail(%d)",__func__ , status);
		seq_printf(buf, "%s\n", "Fail");	
	}
	else{
		seq_printf(buf, "%d\n", cross_talk>>6);	
...
int dump_ST_debug_value1_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value1_read, NULL);
}

const struct file_operations dump_ST_debug_value1_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value1_open,
	.write = dump_ST_debug_value1_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static ssize_t dump_ST_debug_value2_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	int val;
	char messages[8]="";
	if (len > 8)	len = 8;
	...
	printk("STlog signal_Kcps(%d)\n",g_signal_Kcps);
	
	return len;
}


int dump_ST_value2_read(struct seq_file *buf, void *v)
{
	int8_t status = 0;
	int offset = 0;
...
int dump_ST_debug_value2_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_value2_read, NULL);
}

const struct file_operations dump_ST_debug_value2_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value2_open,
	.write = dump_ST_debug_value2_write,	
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
int dump_ST_value3_read(struct seq_file *buf, void *v)
{
	int8_t status = 0;
...
static ssize_t dump_ST_debug_value3_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	int val;
	char messages[8]="";
	if (len > 8)	len = 8;
	...
	printk("STlog sigma_mm(%d)\n",g_sigma_mm);
	
	return len;
}	
int dump_ST_debug_value3_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_value3_read, NULL);
}

const struct file_operations dump_ST_debug_value3_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value3_open,
	.write = dump_ST_debug_value3_write,		
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int dump_ST_debug_value4_read(struct seq_file *buf, void *v)
{
...
static ssize_t dump_ST_debug_value4_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	char messages[8]="";
	int val;
	...
	printk("STlog PulsePeriod(%d, %d)\n",g_preRange,g_finalRange);
	
	return len;
}


static int dump_ST_debug_value4_open(struct inode *inode, struct  file *file)
{

	return single_open(file, dump_ST_debug_value4_read, NULL);
}

static const struct file_operations dump_ST_debug_value4_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value4_open,
	.write = dump_ST_debug_value4_write,	
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int dump_ST_debug_value5_read(struct seq_file *buf, void *v)
{
...
static int dump_ST_debug_value5_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value5_read, NULL);
}

static const struct file_operations dump_ST_debug_value5_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value5_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int dump_ST_debug_value6_read(struct seq_file *buf, void *v)
{
	int8_t status = 0;
...
static int dump_ST_debug_value6_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value6_read, NULL);
}

static const struct file_operations dump_ST_debug_value6_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value6_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static int dump_ST_debug_value7_read(struct seq_file *buf, void *v)
{	
	int8_t status = 0;
	uint32_t refKdata[REF_K_DATA_NUMBER];
	status = Sysfs_read_Dword_seq(LASER_K_REF_FILE,refKdata,REF_K_DATA_NUMBER);
...
static int dump_ST_debug_value7_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value7_read, NULL);
}

static const struct file_operations dump_ST_debug_value7_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value7_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int dump_ST_debug_value8_read(struct seq_file *buf, void *v)
{
...
static int dump_ST_debug_value8_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value8_read, NULL);
}

static const struct file_operations dump_ST_debug_value8_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value8_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int dump_ST_debug_value9_read(struct seq_file *buf, void *v)
{
	int8_t status = 0;
	uint32_t refKdata[REF_K_DATA_NUMBER];
	status = Sysfs_read_Dword_seq(LASER_K_REF_FILE,refKdata,REF_K_DATA_NUMBER);
...
static int dump_ST_debug_value9_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value9_read, NULL);
}

static const struct file_operations dump_ST_debug_value9_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value9_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int dump_ST_debug_value10_read(struct seq_file *buf, void *v)
{

	return 0;
}

static int dump_ST_debug_value10_open(struct inode *inode, struct  file *file)
{
	return single_open(file, dump_ST_debug_value10_read, NULL);
}

static const struct file_operations dump_ST_debug_value10_fops = {
	.owner = THIS_MODULE,
	.open = dump_ST_debug_value10_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
extern const struct file_operations ST_I2C_status_check_fops; 
extern const struct file_operations I2C_status_check_fops; 

extern const struct file_operations ST_device_enable_fops;
extern const struct file_operations ST_get_range_fos; 
extern const struct file_operations ST_get_range_more_info_fos; 

extern const struct file_operations ST_device_calibration_fops; 

extern const struct file_operations dump_ST_register_fops; 
extern const struct file_operations dump_ST_debug_register_fops;

extern const struct file_operations dump_ST_value_check_fops;


void ST_create_proc_file(void)
{
	proc(STATUS_PROC_FILE, 0664, &ST_I2C_status_check_fops);
	proc(STATUS_PROC_FILE_FOR_CAMERA, 0664, &I2C_status_check_fops);
	
	
	proc(DEVICE_TURN_ON_FILE, 0660, &ST_device_enable_fops);
	proc(DEVICE_GET_VALUE, 0664, &ST_get_range_fos);	
	proc(DEVICE_GET_VALUE_MORE_INFO, 0664, &ST_get_range_more_info_fos);

	
	proc(DEVICE_SET_CALIBRATION, 0660, &ST_device_calibration_fops);


	//for debug+
	proc(DEVICE_DUMP_REGISTER_VALUE, 0664, &dump_ST_register_fops);
	proc(DEVICE_DUMP_DEBUG_VALUE, 0664, &dump_ST_debug_register_fops);	
	proc(DEVICE_LOG_CTRL_FILE, 0664, &laser_focus_log_contorl_fops);
	//for debug-

	//for ce catch part of K data+
	proc(DEVICE_DEBUG_VALUE1, 0664, &dump_ST_debug_value1_fops);
	proc(DEVICE_DEBUG_VALUE2, 0664, &dump_ST_debug_value2_fops);
	proc(DEVICE_DEBUG_VALUE3, 0664, &dump_ST_debug_value3_fops);
	proc(DEVICE_DEBUG_VALUE4, 0664, &dump_ST_debug_value4_fops);
	proc(DEVICE_DEBUG_VALUE5, 0664, &dump_ST_debug_value5_fops);	
	proc(DEVICE_DEBUG_VALUE6, 0664, &dump_ST_debug_value6_fops);
	proc(DEVICE_DEBUG_VALUE7, 0664, &dump_ST_debug_value7_fops);
	proc(DEVICE_DEBUG_VALUE8, 0664, &dump_ST_debug_value8_fops);
	proc(DEVICE_DEBUG_VALUE9, 0664, &dump_ST_debug_value9_fops);
	proc(DEVICE_DEBUG_VALUE10, 0664, &dump_ST_debug_value10_fops);
